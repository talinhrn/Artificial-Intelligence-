...
def constructBayesNet(gameState: hunters.GameState):
    ...
    obs_val = []
    for i in range(X_RANGE + Y_RANGE - 2 + MAX_NOISE + 1):
        obs_val.append(i) 
    ...
    variableDomainsDict[OBS0] = obs_val
    variableDomainsDict[OBS1] = obs_val
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factors, new_factor = joinFactorsByVariable(factors, variable)
            
            if len(new_factor.unconditionedVariables()) != 1:
                factors.append(eliminate(new_factor, variable))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            return 1 if jailPosition == ghostPosition and noisyDistance is None else 0
    
        # Ghost is not in jail
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            dist[position] = prob * self.beliefs[position]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        current position is known. CHANGE THİS PART TOO
        """
        ...
        for Pos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                updated_beliefs[newPos] += prob * newPosDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        numLegalPositions = len(legalPos)
        self.particles = []

        for var in range(self.numParticles):
            pos = legalPos[var % numLegalPositions]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            if particle not in weightDist:
                weightDist[particle] = 0
